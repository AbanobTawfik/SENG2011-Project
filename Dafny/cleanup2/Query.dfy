...
method getNonExpiredBlood(inv: array<Blood>, currentDate: int) returns (result: array<Blood>)
    requires inv != null;
    requires forall i | 0 <= i < inv.Length :: inv[i] != null;
    ensures  result != null;
    ensures  result != inv;
    ensures  fresh(result);
    ensures  result[..] == getNonExpiredBloodSeq(inv, currentDate);
    ensures  multiset(result[..]) <= multiset(inv[..]);
{
    result := Filter(inv, (b: Blood) requires b != null reads b => !b.IsExpired(currentDate));
}
...
function countBloodByTypeUpTo(inv: array<Blood>, upTo: int, bloodType: string): int
    reads inv;
    requires inv != null;
